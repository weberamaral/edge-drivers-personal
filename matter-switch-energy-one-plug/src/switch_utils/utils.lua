...
function utils.detect_bridge(device)
  -- SmartThings "bridge" detection historically relied on the Aggregator device type being present.
  -- Some bridges (e.g. HA matterbridge) expose BridgedDeviceBasicInformation but may not expose Aggregator,
  -- which makes the stock detection return false and can cause profile re-matching and missing UI states.
  local has_aggregator = #utils.get_endpoints_by_device_type(device, fields.DEVICE_TYPE_ID.AGGREGATOR) > 0
  local has_bridged_basic = false
  if clusters and clusters.BridgedDeviceBasicInformation and clusters.BridgedDeviceBasicInformation.ID then
    has_bridged_basic = #device:get_endpoints(clusters.BridgedDeviceBasicInformation.ID) > 0
  else
    -- fallback to cluster id 0x0039
    has_bridged_basic = #device:get_endpoints(0x0039) > 0
  end
  return has_aggregator or has_bridged_basic
end
