...
function SwitchLifecycleHandlers.do_configure(driver, device)
  if device.network_type == device_lib.NETWORK_TYPE_MATTER then
    ...
    -- Do NOT re-profile bridged devices; keep fingerprint-selected profile stable
...
function SwitchLifecycleHandlers.driver_switched(driver, device)
  if device.network_type == device_lib.NETWORK_TYPE_MATTER then
    -- IMPORTANT: run even for bridged devices
    switch_utils.handle_electrical_sensor_info(device)

    -- Only non-bridged gets dynamic re-profile
    if not switch_utils.detect_bridge(device) then
      device_cfg.match_profile(driver, device)
    end
  end
...
function SwitchLifecycleHandlers.device_init(driver, device)
  if device.network_type == device_lib.NETWORK_TYPE_MATTER then
    ...
    -- IMPORTANT: run electrical sensor setup here too (bridged devices are timing-sensitive)
    -- This is the same routine that makes power/energy work reliably.
    switch_utils.handle_electrical_sensor_info(device)

    -- Determine whether cumulative reports are supported
    ...
    -- Initial reads with retry to avoid "device not fully updated" after pairing/driver swap
    local function read_initial()
      device:send(clusters.OnOff.attributes.OnOff:read(device))
      device:send(clusters.ElectricalPowerMeasurement.attributes.ActivePower:read(device))
      device:send(clusters.ElectricalEnergyMeasurement.attributes.CumulativeEnergyImported:read(device))
    end

    read_initial()
    driver:call_with_delay(2, read_initial)
    driver:call_with_delay(6, read_initial)
  else
    -- For CHILD devices, keep the original behavior (if you have any)
    device:try_update_metadata({ profile = "plug-power-energy-powerConsumption" })
  end
end
